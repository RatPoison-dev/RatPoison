...
class AimPistolTab : Tab(true, false) { //Aim.kts tab
    ...
    val aimFovLabel = VisLabel("Aim Fov: $PISTOL_AIM_FOV" + when(PISTOL_AIM_FOV.toString().length) {3->"  " 2->"    " else ->"      "}) //Aim_Fov
    val aimFovSlider = VisSlider(1F, 360F, 2F, false) //Aim_Fov
    val aimSpeedMinLabel = VisLabel("Aim Speed Min: $PISTOL_AIM_SPEED_MIN" + when(PISTOL_AIM_SPEED_MIN.toString().length) {3->"  " 2->"    " else ->"      "}) //Aim_Speed_Min
    val aimSpeedMinSlider = VisSlider(1F, 100F, 1F, false) //Aim_Speed_Min
    val aimSpeedMaxLabel = VisLabel("Aim Speed Max: $PISTOL_AIM_SPEED_MAX" + when(PISTOL_AIM_SPEED_MAX.toString().length) {3->"  " 2->"    " else ->"      "}) //Aim_Speed_Max
    ...
    init {
        ...
        aimBoneBox.changed { _, _ ->
            if (aimBoneBox.selected.toString() == "HEAD_BONE") {
                PISTOL_AIM_BONE = HEAD_BONE
            }
            else if (aimBoneBox.selected.toString() == "BODY_BONE") {
                PISTOL_AIM_BONE = BODY_BONE
            }
        }
        ...
        //val aimFovLabel = VisLabel("Aim Fov: " + PISTOL_AIM_FOV.toString() + when(PISTOL_AIM_FOV.toString().length) {3->"  " 2->"    " else ->"      "}) //Need alternative for all of these, has to be some kind of setup for it
        ...
        //val aimSpeedMinLabel = VisLabel("Aim Speed Min: " + PISTOL_AIM_SPEED_MIN.toString() + when(PISTOL_AIM_SPEED_MIN.toString().length) {3->"  " 2->"    " else ->"      "})
        ...
        //val aimSpeedMaxLabel = VisLabel("Aim Speed Max: " + PISTOL_AIM_SPEED_MAX.toString() + when(PISTOL_AIM_SPEED_MAX.toString().length) {3->"  " 2->"    " else ->"      "})
        ...
        //val aimStrictnessLabel = VisLabel("Aim Strictness: " + PISTOL_AIM_STRICTNESS.toString()) //Doesnt need when as it stays 1.0 through 5.0
        ...
        //val perfectAimFovLabel = VisLabel("Perfect Aim Fov: " + PISTOL_PERFECT_AIM.toString() + when(PISTOL_PERFECT_AIM.toString().length) {3->"  " 2->"    " else ->"      "})
        ...
        //val perfectAimChanceLabel = VisLabel("Perfect Aim Chance: " + PISTOL_PERFECT_AIM_CHANCE.toString() + when(PISTOL_PERFECT_AIM_CHANCE.toString().length) {3->"  " 2->"    " else ->"      "})
        ...
        if (PISTOL_AIM_ASSIST_MODE) aimAssistCheckBox.isChecked
        //val aimAssistTable = VisTable()
        //val aimAssistCollapsible = CollapsibleWidget(aimAssistTable)
        aimAssistCollapsible.isCollapsed = !PISTOL_AIM_ASSIST_MODE//setCollapsed(!PISTOL_AIM_ASSIST_MODE)
        ...
        //val aimAssistStrictnessLabel = VisLabel("Aim Assist Strictness: " + PISTOL_AIM_ASSIST_STRICTNESS.toString() + when(PISTOL_AIM_ASSIST_STRICTNESS.toString().length) {3->"  " 2->"    " else ->"      "})
        ...
        aimAssistStrictnessSlider.changed { _, _ ->
            ...
            aimAssistStrictnessLabel.setText("Aim Assist Strictness: " + PISTOL_AIM_ASSIST_STRICTNESS.toString() + when(PISTOL_AIM_ASSIST_STRICTNESS.toString().length) {3->"  " 2->"    " else ->"      "}) //When is used to not make the sliders jitter when you go from 10 to 9, or 100 to 99, as that character space shifts everything, one character is 2 spaces
        }
