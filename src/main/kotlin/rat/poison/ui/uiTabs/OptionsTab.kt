...
class OptionsTab : Tab(false, false) {
    ...
    val closeWithCSGO = VisCheckBoxCustom("Close With CSGO", "CLOSE_WITH_CSGO")
    ...
    init {
        ...
        table.add(closeWithCSGO).padLeft(25F).left().row()
...
fun updateWindows() {
    if (VisUI.isLoaded()) {
        uiBombWindow.updatePosition(curSettings.float["BOMB_TIMER_X"], curSettings.float["BOMB_TIMER_Y"]) //TODO remove this bs
