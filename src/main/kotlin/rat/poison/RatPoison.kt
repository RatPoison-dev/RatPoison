...
// Override Weapon
data class OverrideWeapon(
    var tOverride: Boolean = false,
    var tFRecoil: Boolean = false,
    var tOnShot: Boolean = false,
    // ... add other properties here
)

// Skinned Weapon
data class SkinnedWeapon(
    var tSkinID: Int,
    var tStatTrak: Int,
    var tWear: Float,
    var tSeed: Int
...
fun main() {
    ...
    if (!curSettings["MENU"].strToBool()) {
        // If we aren't using the menu, disable everything that uses the menu
        ...
        if (dbg) println("[DEBUG] Initializing Recoil Ranks")
        ranks()

        if (dbg) println("[DEBUG] Initializing Recoil Spectator List")
        spectatorList()

        if (dbg) println("[DEBUG] Initializing Recoil Bomb Timer")
        bombTimer()

        if (dbg) println("[DEBUG] Initializing Recoil Crosshair")
        rcrosshair()

        if (dbg) println("[DEBUG] Initializing Hit Marker")
        hitMarker()

        if (dbg) println("[DEBUG] Initializing Nade Helper")
        nadeHelper()

        if (dbg) println("[DEBUG] Initializing Nade Tracer")
        nadeTracer()

        if (dbg) println("[DEBUG] Initializing Draw Fov")
        drawFov()

        if (dbg) println("[DEBUG] Initializing Spread Circle")
        spreadCircle()

        if (dbg) println("[DEBUG] Initializing Draw Smokes")
        drawSmokes()

        if (dbg) println("[DEBUG] Initializing Far Radar")
        farRadar()

        drawDebug()
    }

    if (dbg) println("[DEBUG] Initializing Bunny Hop")
    bunnyHop()

    if (dbg) println("[DEBUG] Initializing Auto Strafe")
    strafeHelper()

    if (dbg) println("[DEBUG] Initializing Kill Bind")
    killBind()

    if (dbg) println("[DEBUG] Initializing RCS")
    rcs()

    if (dbg) println("[DEBUG] Initializing Flat Aim")
    flatAim()

    if (dbg) println("[DEBUG] Initializing Path Aim")
    pathAim()

    if (dbg) println("[DEBUG] Initializing Set Aim")
    setAim()

    if (dbg) println("[DEBUG] Initializing Bone Trigger")
    triggerBot()

    if (dbg) println("[DEBUG] Initializing Auto Knife")
    autoKnife()

    if (dbg) println("[DEBUG] Initializing Reduced Flash")
    reducedFlash()

    if (dbg) println("[DEBUG] Initializing ESPs")
    esp()

    if (dbg) println("[DEBUG] Initializing Esp Toggle")
    espToggle()

    if (dbg) println("[DEBUG] Initializing Fast Stop")
    fastStop()

    if (dbg) println("[DEBUG] Initializing Head Walk")
    headWalk()

Continued:

```kotlin
    if (dbg) println("[DEBUG] Initializing Adrenaline")
    adrenaline()

    if (dbg) println("[DEBUG] Initializing FovChanger")
    fovChanger()

    if (dbg) println("[DEBUG] Disabling Post Processing")
    disablePostProcessing()

    if (dbg) println("[DEBUG] Initializing Door Spam")
    doorSpam()

    if (dbg) println("[DEBUG] Initializing Weapon Spam")
    weaponSpam()

    if (dbg) println("[DEBUG] Initializing Weapon Changer")
    skinChanger()

    if (dbg) println("[DEBUG] Initializing NightMode/FullBright")
    nightMode()

    if (dbg) println("[DEBUG] Initializing Bomb Updater")
    bombUpdater()

    if (dbg) println("[DEBUG] Initializing Handle Fire Key")
    handleFireKey()

    if (dbg) println("[DEBUG] Initializing Head Level Helper")
    headLevelHelper()

    if (dbg) println("[DEBUG] Initializing Nade Thrower")
    nadeThrower()

    if (dbg) println("[DEBUG] Initializing Name Changer")
    nameChanger()
    ...
    if (curSettings["MENU"].strToBool()) {
        ...
        GlobalScope.launch {
            ...
            Lwjgl3Application(App, Lwjgl3ApplicationConfiguration().apply {
                ...
                useVsync(false)
                glfwSwapInterval(0)
                glfwWindowHint(GLFW.GLFW_DOUBLEBUFFER, GLFW.GLFW_TRUE)
