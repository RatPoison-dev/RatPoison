...
internal fun radarEsp() = every(100, inGameCheck = true) {
    ...
    if (curSettings.bool["LEGIT_RADAR"]) {
        ...
        for (i in footSteps.indices) {
            ...
            if (ent > 0L) {
                entsChecked.add(ent)
                if (!footSteps[i].open && footSteps[i].ttl > 0 && Vector(footSteps[i].x, footSteps[i].y, footSteps[i].z).distanceTo(me.position()) <= curSettings.int["LEGIT_RADAR_FOOTSTEPS_DISTANCE"]) {
                    ent.showOnRadar()
                } else {
                    ent.hideOnRadar()
                }
