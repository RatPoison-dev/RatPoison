	...
	App {
		...
		forEntities(forEntsList, iterateWeapons = true) { //Player & Weapon boxes
			...
			if (bEspName && isWeapon) {
				...
				drawTextureOrText(assetManager, bEspUseIcons, name, name.toLocale(), bEspNamePos, true)
			}
			...
			if (bEspWeapon && isPlayer) {
				...
				drawTextureOrText(assetManager, bEspUseIcons, weaponName, weaponName.toLocale(), bEspWeaponPos, true)
			}
